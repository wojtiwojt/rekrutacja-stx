...
def add_or_edit_book_view(request, pk=None):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(request, f'Pomyślnie zapisano tytuł "{form.cleaned_data["title"]}".')
            return redirect("/")
...
def list_of_books_view(request):
    ...
    if request.GET:
        ...
        if title or author or language or date_start or date_end:
            ...
            messages.success(request, f'Liczba wyszukań dla wybranych kryteriów: {len(books)}')
            paginator = Paginator(books, 5)
            page_number = request.GET.get('page')
            page_obj = paginator.get_page(page_number)
            return render(request, "book_list.html", {'page_obj': page_obj, 'books_count': all_books_count, 'books_in_search': len(books)})
    
    books = Book.objects.all().order_by('title')
    paginator = Paginator(books, 5)
    page_number = request.GET.get('page')
    page_obj = paginator.get_page(page_number)
    return render(request, "book_list.html", {'page_obj': page_obj, 'books_count': all_books_count})
            ...
            if data_json['totalItems'] > 0:
                for item in data_json['items']:
                    ...
                    for volume_info in item['volumeInfo']:
                        ...
                        if volume_info == 'industryIdentifiers':
                            for identifier in item['volumeInfo'][volume_info]:
                                if IndustryIdentifiers.objects.filter(identifier=identifier['identifier']).exists():
                                    identifier_type = identifier['type']
                                    identifier_id = identifier['identifier']
                                    existing_identifiers_in_db.append(f'{identifier_type} - {identifier_id}')
                                if identifier['type'] == "ISBN_10":
                                    industry_identifiers.append({'id_type': 'isbn_10', 'identifier': identifier['identifier']})
                                if identifier['type'] == "ISBN_13":
                                    industry_identifiers.append({'id_type': 'isbn_13', 'identifier': identifier['identifier']})
                                if identifier['type'] == "OTHER":
                                    industry_identifiers.append({'id_type': 'other_id', 'identifier': identifier['identifier']})
                        if volume_info == 'imageLinks':
                            if item['volumeInfo'][volume_info]['smallThumbnail']:
                                book.cover_url = item['volumeInfo'][volume_info]['smallThumbnail']
                        if volume_info == 'language':
                            for language in languages_json:
                                if item['volumeInfo'][volume_info].lower() == language['value']:
                    ...
                    if not existing_identifiers_in_db:
                        ...
                        if errors:
                            errors.append(f'id książki z błedami: {book.id}, Tytuł "{book.title}" Pomimo to, tytuł został dodany.')
                        ...
                        messages.warning(request, f'Nie zaimportowano ksiązki "{book.title}" z powodu instenijących w bazie numerów isbn: {existing_identifiers_in_db}. Prawdopodobnie ten tytuł znajduje się w bazie.')
            else:
                messages.warning(request, f'Nie znaleziono wyników dla wybranych słów kluczowych.')
        messages.success(request, f'Liczba zaimportowanych książek: {books_imported_count}')
        print(all_errors)
        return redirect("/")
    return render(request, 'import.html')
